...
const productionPaths = ['components', 'functions', 'lib', 'scripts', 'src', 'vite.config.ts'];
...
function checkDuplication() {
  const outputDirectory = mkdtempSync(join(tmpdir(), 'anime-list-jscpd-'));
  run('pnpm', [
    'exec',
    'jscpd',
    ...productionPaths,
...
function checkSuppressions() {
  const files = ['components', 'functions', 'lib', 'scripts', 'src', 'e2e'].flatMap((root) =>
